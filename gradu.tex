...
\colorbox{YellowGreen}{// TODO: Mieti tämän sijoittelua}

Tutkielman tavoitteena oli tutkia progresiivisia WWW-sovelluksia ja niiden sopimista avusteisen kommunikaation mobiilisovellusten tuottamiseen sekä ohjelmistoteknisestä, että käytettävyysnäkökulmasta. Tutkielmaa varten luotiin suunnittelutieteellisen tutkimuksen (engl. \textit{design study}) periaatteiden mukaisesti artefakti, FreeAAC-ohjelmisto, jota verrattiin ohjelmistotekniikan ja käytettävyystutkimuksen heuristiikkoihin. Käytettävyystutkimuksessa otettiin huomioon erityisryhmien tarpeet ja etenkin autististen käyttäjien käytettävyyshaasteet. 
...
Tyydyttävyys mittaa käyttöliittymän tuottamaa mielihyvää käyttäjässä. Tyydyttävyyttä ymmärtääkseen täytyy tietää, että käyttöliittymä ja käyttäjäkokemus ovat kaksi eri asiaa. Käyttäjäkokemus ja sen tyydyttävyys tai tyydyttämättömyys on seurausta useasta eri tekijästä. Visuaalinen ulkoasu on osatekijä käyttöliittymän tyydyttävyyttä arvioidessa, mutta myös halutuilla ominaisuuksilla ja niiden toimivuudella on merkitystä tyydyttävyyteen. Jos käyttäjä ei löydä haluamiaan toimintoja, osa halutuista ominaisuuksista puuttuu tai ne on hankalta löytää, ohjelmiston tyydyttävyys on matala. Tyydyttävyys on viidestä edellä listatusta käytettävyystekijästä kaikista subjektiivisin.
...
\textbf{ESLint} on koodianalyysityökalu, joka käy läpi ohjelmistoympäristössä olevaa ohjelmakoodia ja auttaa ohjelmoijaa ohjelmoimaan koodianalyysityökaluun valittujen asetusten mukaisesti.
...
\colorbox{YellowGreen}{// TODO: Pitäisikö koodirivijutut siirtää edelliseen osaan?}
...
Rivien kokonaismäärä on 8848. Tiedostojen rivien lukumäärät:
\begin{center}
    \begin{tabular}{ | l | l |}
    \hline
    Tiedosto & Rivien lukumäärä \\ \hline
...
Ohjelmiston ainoa JavaScript-tiedosto on \texttt{service-worker.js}. \texttt{service-worker.js} on oleellinen osa progressiivisen WWW-sovelluksen toimintaa sillä se mahdollistaa toimintoja, jotka ovat aiemmin olleet mahdollisia vain natiiveille sovelluksille. Se on itsenäinen komentosarja, joka voi suorittaa toimintoja, joihin ei tarvita käyttäjän aktiivista osallistumista. Tällaisia ovat muunmuassa Internetiin yhteydessä olevasta sovelluksesta palvelimelle lähtevien kutsujen perille menemisen varmistaminen ja push-ilmoitusten näyttäminen.

Suurin osa riveistä sijaitsee ohjelmistokehittäjän itse tuottamissa mallitiedostoissa ja näitä hallinnoivissa komponenttitiedostoissa. Näissä tiedostoissa olevien rivien määrä on 783. Jos 7528 riviä sisältävä \texttt{package.json}-tiedosto otetaan pois laskuista, on ohjelmiston kokonaisrivimäärä 1320 ja ohjelmistokehittäjän pääasiallisesti tuottamat tiedostot sisältävät noin 59,32\% ohjelmiston kokonaisrivimäärästä.
...
Tehokkuus on määre, jota parhaiten voidaan mitata suorittamalla käyttäjätutkimus, jossa käyttäjälle annetaan tehtävä, johon kulunutta aikaa tai välivaiheiden määrää mitataan. Koska tässä tutkielmassa ei suoritettu käyttäjätutkimusta vaan suunnittelutieteelinen tutkimus, on tehokkuutta tutkittava heuristiikkojen perusteella.
